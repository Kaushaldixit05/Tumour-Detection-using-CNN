...
class Gui:
    ...
    def check(self):
        ...
        if (self.val.get() == 1):
            ...
            if res > 0.5:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Bhai Respawn hone ka time agaya", height=1, width=20)
                resLabel.configure(background="White", font=("Comic Sans MS", 16, "bold"), fg="red")
            else:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Revive mil jayega tension mat le", height=1, width=20)
